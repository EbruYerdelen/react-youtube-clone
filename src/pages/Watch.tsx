...
export default function Watch() {
  ...
  /*
How useParams Works
Route Definition with Parameters: When defining routes, you can specify parameters in the path using the colon : followed by the parameter name. For example, in the path /users/:userId, userId is a parameter.

Accessing Parameters: Inside the component rendered by a route, you can use the useParams hook to access these parameters.
  */
  ...
  useEffect(() => {
    if (id) {
      dispatch(getVideoDetails(id))//we're dispatching an action creator to the store,so that action creator can find the related reducer and tell it what to do.This action creator will be async thunk cuz it'll fetch api
  ...
  useEffect(() => {
    if (currentPlaying && id) {
      dispatch(getRecommendedVideos(id)); //we're dispatching an action creator to the store,so that action creator can find the related reducer and tell it what to do.this will be similarly async thunk
    }
